...
Our goal is to find the set of robot poses $\x = \begin{bmatrix} \x_1, \x_2, \dots, \x_N \end{bmatrix}^{\top} \in \mathbb{R}^{cN}$ that minimizes the total squared error, the $\rchi^2$ cost function:
\begin{align}
    \rchi^2(\x) = \sum_{e_j \in \mathcal{E}} \mathbf{e}_j(\x)^\top \Omega_j \mathbf{e}_j(\x).
\end{align}
...
Substituting the linearized error into the $\rchi^2$ cost function, we get a quadratic approximation of the error around $\xk$:
\begin{align}
\rchi^2(\xkk) &\approx \sum_{e_j \in \mathcal{E}} (\mathbf{e}_j(\xk) + \mathbf{J}_j \Delta \xk)^\top \Omega_j (\mathbf{e}_j(\xk) + \mathbf{J}_j \Delta \xk) \\
...
&= \rchi_k^2 + 2 \mathbf{b}^T \Delta \xk + (\Delta \xk)^T \Hessian \Delta \xk,
\end{align}
...
\begin{align}
\frac{\partial \rchi^2(\xkk)}{\partial \Delta \xk} = 2 \mathbf{b} + 2 \Hessian \Delta \xk = 0.    
\end{align}
...
\begin{align}
\rchi^{2}(\x)=\sum_{\mathbf{e}_j \in \mathcal{E}}\rho(\mathbf{e}_j^\top \Omega_j \mathbf{e}_j)
\end{align}
...
s &= \min\Bigg(1, \frac{2\Phi}{\Phi+\rchi_{\text{lc}}^2}\Bigg)
\end{align}
where $\mathbf{e}_{\text{lc}}$ is the loop closure constraint, $\Omega_{\text{lc}}$ is the loop closure information matrix, $\Phi$ is a parameter defining the influence of the down-rate and $\rchi_{\text{lc}}^2$ is the original term for the loop closure constraint.
